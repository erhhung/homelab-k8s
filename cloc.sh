...
# into temporary directory "./cloc/", then decrypts encrypted
# files, including .age and Ansible Vault files, decompresses
# .gz and unpacks .tar files (applies all 3 steps in the case
# of .tar.gz.age files), before running `cloc` to produce the
# most accurate stats possible
...
unpack_files() {
  ...
  while read -r path; do
    ...
    case "$path" in
      # *.age files will always be *.gz.age
      # in this project because *.age files
      # are smaller, so large files will be
      # gzipped first, and small files will
      # be Vault-encrypted directly
      *.gz.age)
        # gzage.sh will REPLACE *.gz.age file
        # with file without .gz.age extension
        ./gzage.sh "$path"
        ;;
      *.gz)
        # *.gz will become file
